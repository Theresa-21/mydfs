    ...
    def new_fat_item(self, dfs_path, file_size):
        ...
        for i in range(nb_blks):
            ...
            # 多副本模式
                ...
                selected_hosts = np.random.choice(HOST_LIST, size=DFS_REPLICATION, replace=False)
                for replica_idx, host in enumerate(selected_hosts):
                    data_pd.loc[i * DFS_REPLICATION + replica_idx] = [i, host, blk_size]
